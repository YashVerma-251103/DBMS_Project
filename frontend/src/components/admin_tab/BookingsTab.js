...
const BookingsTab = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:5000/bookings/search');
  ...
  const handleSubmit = async (e) => {
    ...
    if (currentBooking) {
      ...
      baseURL = 'http://localhost:5000/bookings/update';
      method = 'PUT';
    } else {
      // Create new booking.
    ...
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        console.log(`${currentBooking ? 'Update' : 'Creation'} successful:`, result);
        alert(`${currentBooking ? 'Booking updated' : 'Booking created'} successfully!`);
        fetchData();
        setEditMode(false);
        setCurrentBooking(null);
      } else {
        const errorData = await response.json();
        console.error(`Error ${currentBooking ? 'updating' : 'creating'} booking:`, errorData);
        alert(`Error ${currentBooking ? 'updating' : 'creating'} booking: ${errorData.message || 'Unknown error'}`);
      }
    } catch (error) {
      console.error('Error:', error);
      alert('Failed to process booking action. Please check console for details.');
    }
  };
  ...
  const handleDelete = async (booking) => {
    if (window.confirm('Are you sure you want to delete this booking?')) {
      try {
        const url = `http://localhost:5000/bookings/delete?booking_id=${booking.booking_id}`;
        ...
        <table>
          ...
          <tbody>
            {bookingsData.map((booking, index) => (
              <tr
                key={index}
                className={currentBooking?.booking_id === booking.booking_id ? 'selected-row' : ''}
