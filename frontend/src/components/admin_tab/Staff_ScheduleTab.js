...
const StaffScheduleTab = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:5000/staff_schedule/search');
  ...
  const handleSearchSchedules = async (e) => {
    ...
    try {
      const response = await axios.get('http://localhost:5000/staff_schedule/search', {
      ...
      setSearchError('Error fetching staff schedule data. Please try again later.');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        console.log(`${currentSchedule ? 'Update' : 'Creation'} successful:`, result);
        alert(`${currentSchedule ? 'Schedule updated' : 'Schedule created'} successfully!`);
        fetchData();
        setEditMode(false);
        setCurrentSchedule(null);
      } else {
        const errorData = await response.json();
        console.error(`Error ${currentSchedule ? 'updating' : 'creating'} schedule:`, errorData);
        alert(`Error ${currentSchedule ? 'updating' : 'creating'} schedule: ${errorData.message || 'Unknown error'}`);
      }
    } catch (error) {
      console.error('Error:', error);
      alert('Failed to process schedule action. Please check console for details.');
    }
  };
        ...
        <table>
          ...
          <tbody>
            {schedulesData.map((schedule, index) => (
              <tr
                key={index}
                className={currentSchedule?.schedule_id === schedule.schedule_id ? 'selected-row' : ''}
              ...
              >
                <td>
                  <input
                    ...
                    checked={currentSchedule?.schedule_id === schedule.schedule_id}
